...
// Number of rows that are not masked out, reduced across the (single warp) block.
// The mask is logits shaped but only the first element of each row carries the flag.
...
static __global__ void cross_entropy_loss_f32(
    ...
    if (masked && mask[int64_t(blockIdx.x)*nclasses] <= 0.5f) {
        // This row contributes nothing, but its dst slot must still be written
        // because the caller sums all of them.
        if (threadIdx.x == 0) {
            dst[blockIdx.x] = 0.0f;
        }
        return;
    ...
    // Masked: normalize by the number of rows that are not masked out. This row is not
    // masked out, so nvalid >= 1 and no zero check is needed.
